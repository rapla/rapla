...
> + ordering). Both are present in both datasets (Deployment A: 121 annotations + 1 config;
> Deployment B: 50 annotations + config). The new GraphQL views replace the **combination**
...
`defaultValue`) — see the example deployment's `appointments` view below.
...
## 2. An example deployment's **configured** `appointments` (Termine) view — ground truth
...
`resources(filter:)`; **projection / derivation** (name, start, end, times, duration,
...
### `appointments` (Termine) — GraphQL query
...
`$filter` variable; the per-column `resources(filter:)` is the **annotation** filter,
inline (the shipped `Appointment.resources(filter: ResourceFilter)`):
...
query Termine_appointments($filter: ReservationFilter!) {
  appointmentBlocks(filter: $filter) {                                              # rows = blocks; $filter ← CalendarModel (incl. from/to)
    ...
    kurs:   resources(filter:{ typeKeyIn:["course","subCourse","courseGroup"] }) { displayName }   # Kurs (annotation → inline)
    person: resources(filter:{ isPersonEq:true })                                { displayName }   # Person
    raum:   resources(filter:{ typeKeyIn:["room","subRoom","virtualRoom"] })     { displayName }   # Raum
...
|---|---|---|---|---|---|---|
| Programmieren II | 15.06.2026 08:00 | 15.06.2026 09:30 | course-A, course-B | Prof. X, Dr. A | Room 101, Room 102 | 2 UE 0 Min |
...
- **Annotation filters → inline (view definition).** The per-column allocatable filter
  (`Kurs` = `typeKeyIn:["course","subCourse","courseGroup"]`, `Raum` = room types, `Person` =
  ...
  **`Appointment.resources(filter: ResourceFilter)`** field ([PRD 073](../prd/073-graphql-function-equivalents.md)
  Phase 0, 2026-06-19).
...
query Termine($filter: ReservationFilter!) {
  appointmentBlocks(filter: $filter) {    # $filter ← CalendarModel, e.g.
        # { from, to, typeKeyEq:"lecture",
        #   whereCourse:{ AND:[{campus:{eq:"X"}},{year:{eq:2024}}] },   # neue Regel für
        #   resourceMatching:{ typeKeyIn:["room","subRoom"], idIn:["room-1"] } }  # resource tree
    name: reservation { displayName }
    kurs: resources(filter:{ typeKeyIn:["course","subCourse","courseGroup"] }) { displayName }   # annotation filter — INLINE
    # … start, end, person, raum, duration …
  }
}
```
`ReservationFilter` is the existing schema input (from/to mandatory, `typeKeyEq`,
`resourceMatching`/`resourceIdsIn`, generated `whereXxx`, `searchText`/`matchKind`,
...
| `appointments` (Name, Beginn, Ende, Kurs, Person, Raum, Dauer) | ✅ flat, 0 directives | `appointmentBlocks(filter:$filter)` + type-split annotation `resources(filter:)` + server-eval `start`/`end`/`duration` |
...
`Appointment.resources(filter:)` (shipped 2026-06-19); a server-evaluated `duration`
...
of the hard-coded `["room","subRoom",…]` lists).
