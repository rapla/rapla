...
**Parent:** [PRD 035 (done) — GraphQL foundations](done/035-graphql-foundations.md) §"Classification axis — cross-type queries" + §"Input vs output groups — both v1". PRD 035 designed this as **the primary cross-type surface** but deferred implementation; this PRD picks up the work.
...
- [PRD 057 (done) — DT mutations v1](done/057-graphql-dt-mutations-v1.md) + [PRD 061 — DT mutations v2](061-graphql-dt-mutations-v2.md) — annotation surface needed to admin-edit group membership; this PRD reads the annotation, PRD 061 owns the writing
- [PRD 059 — Typed Where Predicates](059-graphql-typed-where-predicates.md) — per-type `<TypeKey>Where` predicate pattern this PRD reuses verbatim for the group filters (`<Group>Filter` is the union shape; `where<TypeKey>` from PRD 059 is per-type)
...
- For each declared group, the generator computes the **intersection** of attribute (key, valueType, multiplicity) across all member DTs and emits:
  ...
  - A typed input `<Group>Filter` reusing the PRD 059 `where<TypeKey>` pattern — predicates over the shared attributes + AND/OR/NOT combinators.
...
- New Query roots per group on the reservation side: `<groupName>s(filter: <Group>Filter, from: LocalDateTime!, to: LocalDateTime!): [<Group>!]!` (e.g. `courseEvents(filter:, from:, to:)`). Same time-window discipline as PRD 055's `reservations(filter:)`.
...
Out of scope:
- Allocatable-side groups (e.g. "all bookable resources implementing `HasCapacity`"). Same design but separate slice — open as PRD 066 if demand surfaces.
...
- Group-aware mutations (`createCourseEvent(...)`). v1 is read-only; writes go through the typed-per-DT path from PRD 056.
- Admin UI for editing group membership. Annotation is read here; editing belongs to PRD 061's annotation-surface expansion.
...
Annotation key: `graphqlGroups` — comma-separated list (whitespace-trimmed). Empty / missing = type belongs to no declared groups (only the structural interface). The list shape is verbatim per PRD 058 key-spec migration (no silent transforms, no PascalCase auto-rewrite).
...
**Intersection rule (locked from PRD 035 §"Shared attributes — declared groups"):** an attribute belongs to the group interface iff every member DT has an attribute with:
- the same KEY
...
1. Resolves the caller from `RequestContextInstrumentation` (per PRD 055 perf pattern).
...
4. Calls `operator.queryAppointmentsByLocalDateTime(caller, allocatables, owners, from, to, classificationFilters, null, false)` — same path PRD 055's `reservations(filter:)` uses.
5. §12-filters the result, applies the 500 default / 5000 cap.

Reuses the existing `ClassificationFilter` execution engine — no new evaluator, no engine rewrite (consistent with PRD 035 §"Execution — no engine rewrite").

### Member-DT changes during hot-swap

When admin saves a DT (PRD 057 / PRD 061), `GraphQlSchemaRebuilder` already polls + rebuilds the schema within ~10s. The group's intersection is recomputed on every rebuild. If admin adds `graphqlGroups: "CourseEvent"` to a new DT, that DT joins the group on the next rebuild. If admin removes a group from a DT, the DT drops out — schema-breaking but the SPA's hot-swap discovery flow (PRD 057 §"Hot-swap visibility") handles this same way it handles concrete `<TypeKey>Classification` regeneration.

### §12 invariants

Identical to PRD 055's `Query.reservations(filter:)`:
1. Anonymous → empty.
...
3. **Phase 3 — input filter emission.** Emit `<Group>Filter` SDL — same shape as PRD 059's `<TypeKey>Where`, restricted to the shared-attribute set. Tier-3 introspection test confirms `CourseEventFilter` has the expected predicates.
...
Designed in PRD 035 §"declared groups" (shown as a hierarchy `Reservation → CourseEvent → Lecture`) but practical hierarchies are admin-pain to maintain. Lean: defer; v1 supports flat multi-membership only (a DT belongs to N groups, but no group extends another).

### OQ4 — Should we ship the runtime-validated `classificationFilter` escape hatch in this PRD too?

PRD 035 §"Query modes" identifies it as the ad-hoc fallback for cross-type queries that don't have a declared group. Useful for power-user / MCP-agent flows.
...
- **2026-05-29** — PRD opened. Picks up PRD 035's "declared groups (chosen)" pattern. Scope: read-only; reservation side; v1 deliberately avoids the runtime escape hatch + connection pagination + admin UI.
