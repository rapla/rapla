...
- **Relationship to PRD 042's two modes** (full comparison + decision tree in PRD 042 § "Three-way comparison — Mode 1 vs Mode 2 vs PRD 039"): this PRD produces **sidecar entities** (`ExternalAppointment` / `AvailabilityWindow`) visible only in conflict detection — events stay invisible outside that path, titles privacy-stripped by default. PRD 042 Mode 1 / Mode 2 produce **Reservations** that appear everywhere reservations appear (lists, schedule grid, reports, search). PRD 039 is the right choice when the external feed represents *availability constraints* on a person and the events themselves aren't rapla business (typical: personal calendars). PRD 042 is the right choice when the external feed represents *the schedule itself* (typical: administrative / organisational calendars). The fundamental axis: "is this event rapla business, or background information that affects my planning?"
