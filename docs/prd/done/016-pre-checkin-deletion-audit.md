...
A deletion-only classifier missed whole classes of damage (duplicated lines, semantic-divergence pairs, caller-side cascades). Final method runs six complementary passes and combines evidence per file:
...
| C | each hunk | a `-` line and high-similarity `+` line with deletions between → move-with-drops |
...
A line/hunk is safe only if **no pass flags it** AND every `-`/`+` pair classifies as `mechanical` (imports/whitespace/braces/comments) or `pure-rename` (clean type swap with operands intact after stripping rename noise).

### Verdict derivation per hunk

1. **mechanical** — every `-`/`+` line is import / whitespace / brace / comment / annotation only.
2. **pure-rename** — `|dels| == |adds|`; pairing each `-` with its highest-jaccard `+` partner, every pair becomes textually equal after stripping rename noise. No operand reorder, no signature change.
3. **logic-bearing** — anything else.

Files with **only** mechanical/pure-rename hunks are auto-accept. Files with any logic-bearing hunk need per-hunk review.
...
- Pattern 3 — listener-pattern boilerplate stripped (`addChangeListener` / `fireFilterChanged` / `getChangeListeners`).
- Pattern 4 — inline `if`/`while` body dropped between adjacent control-flow keywords.
- Pattern 5 (new, Pass B) — **duplicated lines.** State-machine pass wrote migrated form before deleting original; delete regex failed. Seed: `FileOperator.java:724`. 14 instances across 10 files (incl. `AbstractRaplaSwingCalendar`, `ReservationHelper`, `SimpleEntity`, `ArchiverServiceImpl`).

**Caller-side cascades** (Pass F BLIND-SPOTs): every `cannot find symbol` for `setDateLocalDateTime`/`setTimestampLocalDateTime` in `RaplaSQL.java` referred to helpers removed/renamed in `AbstractTableStorage` without caller updates. Invisible to passes A–E because no `-` line exists at the call site.

## Plan

### Phase 1 — Auto-accept commit (DONE, commit `670219d9`)

67 mechanical/pure-rename files committed in one batch. Compile-clean before commit. Remaining 175 candidates stay in working tree.

### Phase 2 — Top-10 logic-bearing files (in progress)

The top 10 by risk score account for ~28% of remaining logic-bearing hunks (~140 of 497):
...
Walk hunks in order; verify `-`/`+` preserves semantics. Restore collateral; keep intentional. Compile per module after each batch.

### Phase 3 — Long-tail (165 files, ~360 hunks)

Files 11–175 by risk score. Most have 1–5 logic-bearing hunks concentrating on type-rename + operand-flip pairs. Spot-check; accept clusters of pure-rename-shaped hunks unless adjacent to flagged lines.

### Phase 4 — Restore Pattern-5 duplicates (14 sites)

Mechanical: delete duplicate `+` line. One-line edit each.

### Phase 5 — Reactor green-build gate

`mvn test` across reactor before final commit. Targeted regressions for restored Pattern-1 (undo/redo) and Pattern-3 (listener) sites.
...
- Per AGENTS.md §1/§5: targeted during session, full reactor only at end.
