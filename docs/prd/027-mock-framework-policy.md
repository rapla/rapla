...
Decide whether `Mockito` (and mock frameworks generally) belong in the rapla test suite, and write the policy down before it drifts. PRD 024 lands three new server endpoints; PRD 025 adds a presenter test harness. Both touch the question. Mockito is on the classpath (transitively via `spring-boot-starter-test`) and used in exactly one test today; without a rule, that count grows to "everywhere" the next time someone is in a hurry.

## Decision

**Default: no mocks of internal rapla types.** Use the real thing — `FacadeTestSupport` (PRD 017) for tier 2, real Spring context + MockMvc for tier 3. The reactor's existing harnesses make "real" cheap enough that the usual speed/fidelity trade-off flips: mocks don't buy speed and they do cost fidelity.
...
2. `@MockBean` / `@SpyBean` in `@SpringBootTest`. Each busts the Spring context cache (fights PRD 007 Phase 2.7). For controller isolation, use a `@TestConfiguration` static class with hand-rolled stub beans (pattern: `StubPanelsConfig` in `PreferencesAdminControllerIntegrationTest`).
3. `@Mock` partial mocks of pure-Java models carved out by PRDs 023 / 024 (`AllocationConflictModel`, `RepeatingRuleValidator`, `RaplaBuilder`, layout strategies). No I/O — construct directly.
...
| Argument-capture / verification | Hand-rolled `Recording*` doubles capture args directly (see `MockMailer.getMailBody()`, `RecordingPanel.lastSavedValue`). PRD 025's `RecordingView<P>` generalises this with a reflective proxy. |
...
|---|---|---|
| Jackson 3 `final`-field bugs (PRD 011 follow-up; 5 fields × different entity types) | `XmlRoundTripTest` (tier 2, real reader/writer) | Green — mock returns whatever test set up; wire format never exercised. |
...
| `FacadeImpl` constructor signature drift (PRD 017 risk #1) | The four `@SpringBootTest` acceptance tests | Mocked `FacadeImpl` doesn't have a constructor; drift never surfaces. |
...
| (future) `docs/architecture/testing.md` | If PRD 022's tree adopts a testing page, fold this policy there too. Not in scope. |

Out of scope: migrating existing tests (nothing currently breaks the policy except the grandfathered case); banning Mockito at the build level (transitive dep stays — this is a code-review rule); PRD 025's `RecordingView<P>` proxy (that's PRD 025's call).
...
| 2 (ongoing) | PRD 024 / 025 / 026 tests land without `Mockito.mock(rapla.*)`. Grep audit at each close-out: `grep -rE "mock\(.*(Facade\|Operator\|Cache\|Permission\|Conflict)\.class\)" rapla-*/src/test`. Expected: zero matches outside the grandfathered file. |
...
2. **Test-author frustration.** Someone hitting an awkward facade setup may feel mocking would be faster. *Mitigation:* the answer is usually "extend `FacadeTestSupport` with the helper you wanted" (`waitFor(Promise)` was promoted this way in PRD 017 Phase 4 follow-up). Document this loop in §13.
...
- **"Adopt ArchUnit to enforce."** Adds a tier-1 test that lints test source — the kind of meta-testing PRD 016 flagged as expensive maintenance. Revisit if violations recur.
- **"Allow `@MockBean` for expensive `@SpringBootTest` collaborators."** Defeats PRD 007 Phase 2.7 context-cache. Right answer is `@TestConfiguration` + hand-rolled stub bean.
...
2. **Long-term home.** AGENTS.md §13 is primary; this PRD is rationale. If `docs/architecture/` grows a testing page under PRD 022, mirror §13 there.
3. **Constrain PRD 026 (Angular)?** Angular uses Jest/Vitest with its own mocking story. This policy is Java-side only. Angular-side mocking is the next PRD's call.
