...
| 2 — `/api/` prefix | **DONE** | `ApiPathPrefixConfig` adds `/api` to all `@RestController` except the 6 explicit exclusions. SpringDoc honours the prefix → `/api/v3/api-docs`. SPA `BASE_PATH=''` (empty) since spec emits absolute `/api/...` paths. Swing client appends `/api` to serverURL. |
