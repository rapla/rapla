...
- **Bonus:** `expand-blocks` endpoint (`AppointmentSpec` → `AppointmentBlockDto[]`) — server-side `Appointment.createBlocks(...)` so Angular doesn't port weekday-flip / exception-skip logic. PRD 026 §B4 hook.
...
Companion of PRD 023: 023 carves pure-Java models out from under Swing; 024 wraps them in `@HttpExchange` interfaces and serves them. Swing keeps calling the in-process model (no behaviour change); Angular calls REST and renders the result.
...
2. **PRD 020 (server-driven admin panels) and PRD 012 (external event import wizard) already ship the pattern.** 024 generalises it to the reservation-edit surface — the largest UI area not yet covered.
...
(1) and (2) wrap pure-Java models already in rapla-core (PRD 023 Phases 2+3 landed 2026-05-11). (3) wraps the existing `RaplaBuilder` + layout strategies (already headless in rapla-core); the server runs them server-side and serializes the result instead of shipping raw entities.
...
- **`/allocatables/search`** — *rejected*: Swing already has the full set loaded; in-memory filtering on a few thousand entries is sub-ms. Angular can pre-fetch or paginate via `/storage/resources` (PRD 009).
...
Endpoints follow the patterns established by PRD 020:

- `@HttpExchange` interfaces in `rapla-core/.../plugin/edit/` (re-using Spring 6 HttpExchange surface, like `PreferencesAdminService`).
- Request/response DTOs as Java 21 `record`s; field-based Jackson 3 serialization (PRD 010).
...
- **No Swing client migration.** Swing keeps the in-process pure models (PRD 023). REST endpoints are for Angular + external integrations.
...
`AllocationOutcome` matches the existing record in `AllocationConflictModel.java` (PRD 023 Phase 3); `RecurrenceRule` corresponds to `RepeatingRuleModel`. Controllers map DTO ↔ model. Nothing new on the rapla-core side.
...
- Thin bodies: DTO → model → DTO. Same shape as PRD 020's `PreferencesAdminController`.
...
Depends on PRD 023 Phase 2 (`RepeatingRuleValidator`, DONE). One endpoint, no facade hit. Lowest-risk first deliverable; flushes out JWT-protected `@HttpExchange` registration boilerplate.
...
Depends on PRD 023 Phase 3 (`AllocationConflictModel`, DONE). Allocatable-bindings map (today built client-side in `AllocatableSelection.setReservation(...)`) is built server-side from requested allocatables + the user's permission scope — the biggest behavioural carve-out.
...
1. **Round-trip latency.** Naive `validateRecurrence` per-keystroke is wasteful. Mitigation: validator is a pure model in rapla-core (PRD 023 P2); TS port is trivial. REST is **authority**; Angular may shadow client-side for UX.
...
3. **DTO drift.** New fields need Jackson field-based migration (PRD 010); use `@JsonProperty` discipline per PRD 020.
...
5. **Time zones.** `LocalDateTime` is timezone-naive (PRD 014); server interprets in own zone. Document in `AppointmentSpec` JavaDoc.
