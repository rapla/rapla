...
- ~~**GraphQL / aggregated queries.** Each surface = one REST endpoint. No combined queries.~~ **Narrowed 2026-05-24 by [PRD 035 (done) — GraphQL foundations](done/035-graphql-foundations.md).** The per-view shape principle still applies (no cross-view aggregation; each surface keeps its narrow projected shape). But GraphQL is now the SPA's transport for per-view reads: the GraphQL `renderedBlocks` field is a thin wrapper over the existing `CalendarLayoutEngine` + `CalendarViewController` substrate this PRD already shipped (Phases 1–6). REST endpoints (`/calendar/view`, `/table/*`, `/export/csv`) stay for direct REST callers (including Swing); CSV/iCal export stays REST permanently (own API surface per PRD 035 §"Out of core GraphQL").
