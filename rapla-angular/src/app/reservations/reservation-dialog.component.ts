...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <dl>
        <dt>Start</dt><dd>{{ data.start | date:'medium' }}</dd>
        <dt>End</dt><dd>{{ data.end | date:'medium' }}</dd>
        <dt>Allocatables</dt><dd>{{ data.allocatables.join(', ') || '—' }}</dd>
      </dl>
  ...
  styles: [`
    ...
    dt { color: rgba(0, 0, 0, 0.6); font-size: 0.85rem; }
    dd { margin: 0; }
    .muted { color: rgba(0, 0, 0, 0.5); font-size: 0.8rem; margin-top: 1rem; }
  `]
})
