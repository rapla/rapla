...
export class QuickEditProtoComponent {
    ...
    this.open(
      {
        ...
        title: '',
        type: 'event',
        start: this.at(day, hour),
        end: this.at(day, hour + 1),
        res: [],
      },
      e,
    );
  ...
  openNew(e: MouseEvent): void {
    this.open(
      { id: null, title: '', type: 'event', start: this.at(0, 10), end: this.at(0, 11), res: [] },
      e,
    );
  }
  ...
  private clampEnd(newEnd: Date): void {
    ...
    if (!q) return;
    const end =
      newEnd.getTime() <= q.start.getTime() ? new Date(q.start.getTime() + 15 * 60e3) : newEnd;
  ...
  saveQuick(): void {
    ...
    this.events.update((list) => {
      if (q.id === null) {
        return [
          ...list,
          { id: this.seq++, title, type: q.type, start: q.start, end: q.end, res: q.res },
        ];
      }
      return list.map((e) =>
        e.id === q.id ? { ...e, title, type: q.type, start: q.start, end: q.end } : e,
      );
    });
    this.closeAll();
  }
