...
@Component({
  ...
  styles: `
    :host { display: block; padding: 16px; max-width: 760px; }
    .hint { color: #5c6774; font-size: 12px; margin: 0 0 12px; }
    .sec { background: #fff; border: 1px solid #d9dee6; border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; }
    ...
    mat-form-field.time { width: 130px; }
    mat-form-field.rep-type { width: 170px; }
    mat-form-field.num { width: 90px; }
    .weekdays { display: flex; gap: 4px; margin: 6px 0 2px; }
    .wd { border: 1px solid #d9dee6; border-radius: 99px; background: #fff; width: 38px; height: 30px; cursor: pointer; font: inherit; font-size: 12px; }
    .wd.on { background: #1a73e8; border-color: #1a73e8; color: #fff; }
    .wd-error { color: #d93025; font-size: 12px; margin: 4px 0 0; }
    .summary { background: #eef1f5; border-radius: 8px; padding: 8px 12px; font-size: 13px; margin-top: 10px; }
    .end-line { display: flex; align-items: center; gap: 8px; min-height: 56px; }
    .preview { display: flex; flex-direction: column; }
    .occ { display: flex; align-items: center; gap: 10px; padding: 5px 8px; border-bottom: 1px dashed #edf0f4; border-radius: 6px; background: none; border-left: none; border-right: none; border-top: none; font: inherit; text-align: left; cursor: pointer; width: 100%; }
    .occ:hover { background: #f2f6fd; }
    .occ .no { color: #5c6774; width: 26px; font-size: 12px; }
    .occ.skipped .when { text-decoration: line-through; color: #9aa4b0; }
    .occ .tag { margin-left: auto; font-size: 11px; border-radius: 99px; padding: 1px 8px; }
    .occ .tag.skip { background: #fce8e6; color: #d93025; }
    .occ .tag.hover-only { visibility: hidden; background: #eef1f5; color: #5c6774; }
    .occ:hover .tag.hover-only { visibility: visible; }
    .more { color: #5c6774; font-size: 12px; padding: 8px; }
    .badge { background: #d93025; color: #fff; border-radius: 99px; font-size: 11px; padding: 0 7px; margin-left: 6px; }
  `,
  template: `
    <h2>Wiederholung — Prototyp (PRD 091 Phase 4.0)</h2>
    <p class="hint">
      Swing-Parität: Typ / Intervall / Wochentage / Ende (nie · Datum · N-mal) / Ausnahmen.
      Klick auf einen Vorschau-Termin überspringt ihn (Ausnahme, UC-E4). Monats-/Jahresmuster
      leiten sich aus dem Starttermin ab — wie im Kernmodell.
    </p>
    ...
    <div class="sec">
      ...
      <div class="row">
        <mat-form-field class="date" appearance="outline">
          ...
          <input matInput [matDatepicker]="sd" [value]="start()" (dateChange)="setStartDate($event.value)" />
          <mat-datepicker-toggle matSuffix [for]="sd" />
          <mat-datepicker #sd />
        </mat-form-field>
        <mat-form-field class="time" appearance="outline">
          <input matInput [matTimepicker]="st" [value]="start()" (valueChange)="setStartTime($event)" />
      ...
      <div class="sec">
        ...
        <div class="preview">
          @for (occ of preview(); track occ.start.getTime(); let i = $index) {
            <button type="button" class="occ" [class.skipped]="occ.skipped" (click)="toggleException(occ)">
          ...
          @if (truncated()) {
            <div class="more">… Vorschau auf {{ previewCap }} Termine gekürzt (Serie läuft weiter).</div>
          }
          @if (!preview().length) {
        ...
        <div class="row" style="margin-top: 8px">
          <button mat-stroked-button [disabled]="rule().endMode === 'FOREVER'" (click)="splitInfo.set(true)">
            In Einzeltermine umwandeln
          </button>
          @if (rule().endMode === 'FOREVER') {
            <span class="hint" style="align-self: center">nur für endliche Serien (Swing-Regel)</span>
          }
          @if (splitInfo()) {
...
export class RepeatingProtoComponent {
  ...
  toggleWeekday(i: number): void {
    this.rule.update((r) => ({
      ...r,
      weekdays: r.weekdays.includes(i) ? r.weekdays.filter((w) => w !== i) : [...r.weekdays, i].sort(),
    }));
  }
  ...
  setEndMode(mode: EndMode): void {
    this.rule.update((r) => ({
      ...r,
      endMode: mode,
      until: mode === 'UNTIL' && !r.until ? new Date(this.start().getTime() + 90 * DAY_MS) : r.until,
    }));
  }
