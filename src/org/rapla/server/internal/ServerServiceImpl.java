...
public class ServerServiceImpl extends ContainerImpl implements StorageUpdateListener, ServerServiceContainer, ServerService, ShutdownService,RemoteServiceDispatcher, RemoteMethodFactory<RemoteServer>,RemoteMethodStub
{
    ...
    public ServerServiceImpl( RaplaContext parentContext, Configuration config, Logger logger) throws RaplaException
    {
        ...
        addWebpage( "raplaclient.xconf",RaplaConfPageGenerator.class);
        addWebpage( "resource",RaplaResourcePageGenerator.class);
        RaplaResourcePageGenerator resource = (RaplaResourcePageGenerator) getWebpage( "resource");
		addContainerProvidedComponentInstance( RaplaResourcePageGenerator.class, resource);
    ...
    public byte[] dispatch(RemoteSession remoteSession, String methodName, Map<String,String> args ) throws Exception
    {
        ...
        try
        {
            ...
            Object[] convertedArgs = remoteMethodService.deserializeArguments(args, method);
            Object result = null;
            try
            {
            ...
            remoteMethodService.serializeReturnValue(user, out, result);
            {
	            Logger debugLogger = getLogger().getChildLogger(interfaceName+"."+ methodName  + ".result" );
	            if ( debugLogger.isDebugEnabled())
	            {
	            	debugLogger.debug(new String(out.toByteArray()));            	
	            }
            }
