...
public class RaplaComponent
{
	...
	private static final String PERMISSION_MODIFY = "permission_modify";
...
/** We check if an attribute with the permission_modify exists and look if the permission is set either globally (if boolean type is used) or for a specific user group (if category type is used)*/
    public static boolean checkClassifiablePermissions(Object object, User user) {
    	Collection<Category> cat = getPermissionGroups(object, dummyCategory);
    	if ( cat == null)
    	{
    		return false;
    	}
    ...
    public static Collection<Category> getPermissionGroups(Object object, Category superCategory) 
	{
            ...
            if ( classification != null)
            {
                ...
                if ( attribute != null)
                {
                    ...
                    if (type2 == AttributeType.BOOLEAN)
                    {
                        ...
                        {
                        	return Collections.singleton(superCategory);
                        }
                        else
                        {
                        	return null;
    ...
    protected <T> UnsupportedOperationException serviceExcption(Object role, RaplaContextException e) {
   ...
   protected UserModule getUserModule() {
	...
	public Reservation copyAppointment(Reservation reservation, Date beginn, Date end) throws RaplaException 
	{
