...
### 15. REST endpoints live under `/api/` — literal prefix on every controller

Every `@RestController` in the rapla server has a class-level `@RequestMapping` whose value starts with `/api/`. The matching client-side proxy interfaces (`@HttpExchange(...)`) carry the same `/api/` prefix in lockstep. No global path-prefix configurer — the wire URL is the literal string on the controller, so `grep '/api/storage'` actually finds the source.
...
**When you add a new REST endpoint:**
